...
<!-- _______________________php login_____________________ -->
		...
		<?php
				...
				if (isset($_SESSION['dorypos_admin1']) && isset($_COOKIE['userlog'])  ) {
		    header('Location: clubmember.php');
			...
			if (isset($_POST['login_submit'])) {
				...
				$user=mysqli_query($db,"SELECT * FROM user WHERE email='$email'");

				if (mysqli_num_rows($user)==1) {
					//got login acces
          echo "dhukse";
					$record=mysqli_fetch_assoc($user);
					 if ($record['phone']==$password) {
						 // creating session
  					 $_SESSION['dorypos_admin1']=$record['id'];
  					 $_SESSION['dorypos_admin_name1']=$record['name'];
  					 $_SESSION['dorypos_root1']=$record['club_id'];
						  // cookie  define
							$c_name=$record['name'];
							$cookie_name='userlog';
							if ($_SESSION['dorypos_root1']==FALSE) {
